...
#$photomove = [photomove]::new("$env:USERPROFILE\Desktop\Mobile-Pics\Test")
#$photomove = [photomove]::new("$env:USERPROFILE\Desktop\Mobile-Pics\Test\Renamed")

$photomove = [photomove]::new("$env:USERPROFILE\Desktop\Mobile-Pics\WhatsApp_Pictures","$env:USERPROFILE\Desktop\Mobile-Pics\Archive_WhatsApp_Pictures")
...
class photomove{
  ...
  [string]$destinationPath = $env:USERPROFILE + "\Desktop\Mobile-Pics\Test\Renamed"
  [string]$destinationFileName
  [string]$path = $env:USERPROFILE + "\Desktop\Phone"
  [string]$jpgDateTaken
  [string]$createDate
  [object]$extensionsToInclude = "*.png","*.gif","*.jpg","*.jpeg","*.mov","*.3gp","*.avi","*.mp4","*.bmp","*.arw"
  ...
  [void] MoveToDestinationPath ($file,$date) {
    ...
    if ($this.createFolders = $true) {
      ...
      $destination = $this.destinationPath + "\$year"
